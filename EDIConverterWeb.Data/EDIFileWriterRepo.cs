...
namespace EDIConverterWeb.Data
{
    public class EDIFileWriterRepo
    {
        ...
        public string WritePurchaseOrderAcknowledgement(int poId)
        {
            ...
            var doc = ctx.PurchaseOrderAcknowledgements.Include(poa => poa.ItemsOrdered).FirstOrDefault(poa => poa.InterchangeId == poId);
            var ediText = $"ISA*00*          *00*          *ZZ*TRISTATE       *ZZ*80040537744CRT *{doc.AcknowledgementDate.ToString("yyMMdd")}*{doc.AcknowledgementDate.ToString("HHmm")}*U*00401*{doc.InterchangeId}*0*{doc.TestIndicator}*>~\n" +
            ...
            foreach(Item item in doc.ItemsOrdered)
            {
